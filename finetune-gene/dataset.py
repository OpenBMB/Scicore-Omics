...
class SupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        transform,
        tokenizer,
        gene_tokenizer, # 新增: 接收一个基因 tokenizer
        slice_config,
        llm_type="minicpm",
        patch_size=14,
        query_nums=64,
        ...
        self.gene_tokenizer = gene_tokenizer # 新增: 保存基因 tokenizer 实例
        ...
        if '<gene>' not in self.tokenizer.vocab:
            self.tokenizer.add_tokens(['<gene>'], special_tokens=True)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        try:
            """is_gene_data = "gene" in self.raw_data[i] and self.raw_data[i]["gene"] is not None

            if is_gene_data:
                # --- 新增: 基因数据处理逻辑 ---

                # 1. 读取 h5ad 文件并提取基因名列表
                adata = anndata.read_h5ad(self.raw_data[i]["gene"])
                gene_names = adata.var_names.tolist()
            
            # 2. 使用 gene_tokenizer 将基因名列表转换为数字ID
                gene_tokens = self.gene_tokenizer(
                    gene_names, 
                    max_length=1500, # 可根据需要调整基因序列的最大长度
                    padding=True, 
                    truncation=True
                )
                gene_input_ids = torch.tensor(gene_tokens['input_ids'], dtype=torch.long)
            
                # 3. 处理对话文本，但保持 <gene> 占位符不变
                conversations = self.raw_data[i]["conversations"]
                ret_text = conversation_to_ids(conversations, self.tokenizer, self.llm_type, max_length=self.max_length)
            
                # 4. 找到 <gene> token 在文本序列中的精确位置，用于后续向量替换
                gene_token_id = self.tokenizer.convert_tokens_to_ids('<gene>')
                gene_indices = torch.where(ret_text['input_ids'] == gene_token_id)[0]
            
                if len(gene_indices) == 0:
                  # 如果对话中没有 <gene> 占位符，这是一个无效样本，跳过
                    logger.warning(f"Sample {i} is a gene sample but has no <gene> token. Skipping.")
                    return self.__getitem__(random.randint(0, len(self) - 1))

                # 5. 构建返回字典，包含文本和基因信息，图像信息则用空值填充
                ret = dict(
                    input_ids=ret_text["input_ids"],
                    position_ids=ret_text["position_ids"],
                    labels=ret_text["target"],
                    attention_mask=torch.ones_like(ret_text["input_ids"], dtype=torch.bool),
                    pixel_values=[],
                    tgt_sizes=torch.empty(0, 2, dtype=torch.long),
                    image_bound=[],
                    gene_input_ids=gene_input_ids,
                    gene_bound=gene_indices.unsqueeze(0)
                )
            ...
            return ret
        except Exception as e: # 最好捕获具体的异常并打印出来
            logger.error(f"Data fetch error at index {i}: {e}", exc_info=True)
            # 增加一个保护机制，防止在只有一个样本且持续出错时无限递归
            if len(self) > 1:
                return self.__getitem__(random.randint(0, len(self) - 1))
            else:
                raise e"""
            ...
            if has_image:
                ...
                # 使用 preprocess 之后的文本 token
                input_ids = image_ret["input_ids"]
                position_ids = image_ret["position_ids"]
                labels = image_ret["target"]
            ...
            if has_gene:
                ...
                # if len(gene_indices) == 0:
                #     logger.warning(f"Sample {i} has gene data but no <gene> token. Skipping.")
                #     return self.__getitem__(random.randint(0, len(self) - 1))
                # gene_bound = gene_indices.unsqueeze(0)
                # gene_indices = tensor([pos])
                if len(gene_indices) == 1:
                    start = gene_indices[0].item()
                    end = start + 1
                    gene_bound = torch.tensor([[start, end]])
...
def data_collator(examples, padding_value=0, max_length=2048, gene_pad_id=0):
    ...
    if has_genes:
        ...
        # 4. 为 Nicheformer 创建 attention_mask，1代表真实基因，0代表填充
        gene_attention_mask = (gene_input_ids != gene_pad_id).long()
    else:
        # 如果整个批次都是图像样本，则基因相关字段为 None
        gene_input_ids = None
