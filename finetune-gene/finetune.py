...
class TrainingArguments(transformers.TrainingArguments):
    ...
    tune_nicheformer: Optional[bool] = field(default=False)
...
def make_supervised_data_module(
    ...
    rank0_print(f"Found {len(file_paths)} data files from pattern: {data_args.data_path}")
    ...
    for path in file_paths:
        try:
            with open(path, "r") as f:
                data = json.load(f)
                train_json.extend(data) # 假设每个json文件包含一个列表
        except Exception as e:
            rank0_print(f"Error loading data from {path}: {e}")
    # --- 结束修改 ---
...
def train():
    ...
    # 解冻 gene_projector
    for param in model.gene_projector.parameters():
        param.requires_grad = True
        
    # (可选) 如果您也想微调 Nicheformer 的一部分，可以解冻它
    # 例如，只解冻最后的几层
    if training_args.tune_nicheformer:
        for layer in model.nicheformer.encoder.layers:
            for param in layer.parameters():
                param.requires_grad = True
            
    # 如果 tune_vision=True，解冻 vision 模块
    if training_args.tune_vision:
        model.vpm.requires_grad_(True)
        model.resampler.requires_grad_(True)

    # 如果 tune_llm=True，解冻 LLM
    ...
    if training_args.use_lora:
        ...
        for name, param in model.llm.named_parameters():
            param.requires_grad = False
        modules_to_save = ['embed_tokens', 'resampler', 'gene_projector']
        if training_args.tune_vision:
    ...
    gene_vocab_path = '/data2/xiaoxinyu/project/model/gene_tokenizer/vocab.json' 
    gene_tokenizer = GeneTokenizer(vocab_file=gene_vocab_path)
