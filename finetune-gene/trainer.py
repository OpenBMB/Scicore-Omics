...
class CPMTrainer(Trainer):
    def __init__(self, *args, **kwargs):
        ...
        if "swanlab" in self.args.report_to:
            # 初始化 SwanLab 实验
            # self.args.run_name 是一个标准的 Trainer 参数，我们可以从 .sh 脚本传入
            swanlab.init(
                project="MiniCPMV-Finetune", # 您在 SwanLab 上的项目名
                experiment_name=self.args.run_name, # 每次运行的名字，例如 "stage1-lr-1e-5"
                config=self.args.to_dict(), # 将所有超参数自动保存到 SwanLab
            )

    def log(self, logs: Dict[str, float], *args, **kwargs) -> None:
        # 将所有接收到的参数原封不动地传递给父类
        super().log(logs, *args, **kwargs)
        # 检查 SwanLab 是否被激活，并且只在主进程 (rank 0) 上记录
        if "swanlab" in self.args.report_to and self.state.is_world_process_zero:
            swanlab.log(logs, step=self.state.global_step)
    # ======================================================
    # ^^^^^^^^^^^^^^^^   您需要添加的代码   ^^^^^^^^^^^^^^^^
    # ======================================================
